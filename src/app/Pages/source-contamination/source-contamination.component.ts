...
@Component({
  selector: 'app-source-contamination',
  templateUrl: './source-contamination.component.html',
  styleUrls: ['./source-contamination.component.sass'],
  styles: ['.hide{\n' +
...
export class SourceContaminationComponent implements OnInit, OnDestroy {

  heading = 'Gestion des source de contamination';
  subheading = 'Enregistrer, modifier et supprimer les sources de contamination du Cameroun . Tous les champs sont obligatoires';
  icon = 'pe-7s-config icon-gradient bg-premium-dark';


  labelTitreFormulaire = 'Informations générales';
  labelButtonSave = 'Enregistrer';
  labelCodeSourceContamination = "Code SourceContamination";
  labelLibelleSourceContaminationFR = "Libellé SourceContamination FR";
  labelLibeleSourceContaminationEN = "Libellé SourceContamination EN";



  labelTitreTableau = 'Liste des sources de contamination enregistrées';
  ...
  valeurID : number;
  valeurCodeSourceContamination : string = '';
  valeurLibelleSourceContaminationFR : string = '';
  valeurLibelleSourceContaminationEN :string = '';
  ...
  listForPaginaition: SourceContaminationM[] = [];
  countItemAffcihe: number = 0;
  page = 1;

  term : string;

  constructor(private serviceSourceContamination: SourceContaminationService,
  ...
  ngOnInit() {
    ...
    this.serviceSourceContamination.getSourceContamination()
      .subscribe(data => {
        this.serviceNotification.configShowNotif.showLoading.show = false;
        this.listForPaginaition = (<SourceContaminationM[]>data);
        this.listSourceContamination = [];
        this.countItemAffcihe = this.listForPaginaition.length;
        this.listSourceContamination = this.listForPaginaition.slice(0, this.LIMIT_PARGINATION);
  ...
  onDeleteSourceContamination(id: number) {
    ...
    valFornotif.then((result) => {
      if (result.value) {
        ...
        this.serviceNotification.configShowNotif.showLoading.show = true;
        this.serviceSourceContamination.deleteSourceContamination(id).subscribe(
          (reponse)=>{
            this.serviceNotification.configShowNotif.showLoading.show = false;
  ...
  onSaveSourceContamination(dataForm) {
    this.serviceNotification.configShowNotif.showModalBlock = true;
    if (this.actionFormulaire == 'Enregistrer') {

      let formatSourceContamination: SourceContaminationM = new SourceContaminationM();

      formatSourceContamination = new SourceContaminationM(null,
        dataForm.codeSourceContamination,
        dataForm.libelleSourceContaminationFR,
        dataForm.libelleSourceContaminationEN
      );

      this.serviceSourceContamination.saveSourceContamination(formatSourceContamination)
        .subscribe(data => {
      ...
      let formatSourceContaminationUpdate: SourceContaminationM = new SourceContaminationM();
      formatSourceContaminationUpdate = new SourceContaminationM(this.valeurID,
        dataForm.codeSourceContamination,
        dataForm.libelleSourceContaminationFR,
        dataForm.libelleSourceContaminationEN
      );
      this.isvisible = true;
      this.serviceSourceContamination.updateSourceContamination(formatSourceContaminationUpdate)
        .subscribe(data => {
          this.serviceNotification.configShowNotif.showModalBlock = false;
  ...
  onSelect(objFormul:SourceContaminationM) {

    this.f.resetForm({
      idOccurentValue : objFormul.id,
      codeSourceContamination : objFormul.codeSourceContamination,
      libelleSourceContaminationFR : objFormul.libelleSourceContaminationFR,
      libelleSourceContaminationEN : objFormul.libelleSourceContaminationEN
    });
   ...
   /* this.valeurCodeSourceContamination = objFormul.codeSourceContamination;
    this.valeurLibelleSourceContaminationFR = objFormul.libelleSourceContaminationFR;
    this.valeurLibelleSourceContaminationEN = objFormul.libelleSourceContaminationEN;*/
  }
  ...
  onChangePageRecup(dataRecup) {
    ...
    if (dataRecup == 1) {
      ...
      this.listSourceContamination = this.listForPaginaition.slice(((dataRecup - 1) * this.LIMIT_PARGINATION), (this.LIMIT_PARGINATION * dataRecup));
    }
  }
  ...
  onChangeNbreDatasAffiche(valNbre){
    ...
    this.page = 1;
    this.listSourceContamination = this.listForPaginaition.slice(0, valNbre);
  }
